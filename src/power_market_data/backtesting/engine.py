...
def backtest_daily(
    ...
    for rows in groups:
        ...
        metrics = DayMetrics(
            ...
            grid_throughput_mwh=charged + discharged,
            cell_throughput_mwh=cell,
            equivalent_full_cycles=cell / (2 * config.energy_capacity_mwh),
            initial_soc_mwh=result.soc_mwh[0],
            terminal_soc_mwh=result.soc_mwh[-1],
            minimum_soc_mwh=min(result.soc_mwh),
            maximum_soc_mwh=max(result.soc_mwh),
            charging_intervals=charging,
            discharging_intervals=discharging,
            idle_intervals=len(rows) - charging - discharging,
        )
        ...
        days.append(
            DayResult(
                ...
                minimum_price_usd_per_mwh=str(min(prices)),
                maximum_price_usd_per_mwh=str(max(prices)),
                inputs=rows,
                dispatch=result,
                metrics=metrics,
            )
    ...
    aggregate = Aggregate(
        total_gross_arbitrage_value_usd=math.fsum(values),
        average_daily_gross_value_usd=statistics.mean(values),
        median_daily_gross_value_usd=statistics.median(values),
        best_day=max(days, key=lambda d: d.dispatch.gross_arbitrage_value_usd).market_date,
        worst_day=min(days, key=lambda d: d.dispatch.gross_arbitrage_value_usd).market_date,
        total_objective_usd=math.fsum(day.dispatch.objective_usd for day in days),
        total_throughput_cost_usd=math.fsum(day.dispatch.throughput_cost_usd for day in days),
        grid_throughput_mwh=math.fsum(day.metrics.grid_throughput_mwh for day in days),
        cell_throughput_mwh=math.fsum(day.metrics.cell_throughput_mwh for day in days),
        equivalent_full_cycles=math.fsum(day.metrics.equivalent_full_cycles for day in days),
        charging_percent=100 * sum(day.metrics.charging_intervals for day in days) / count,
        discharging_percent=100 * sum(day.metrics.discharging_intervals for day in days) / count,
        idle_percent=100 * sum(day.metrics.idle_intervals for day in days) / count,
    )
